<script lang="ts">
	...
	onMount(() => {
		...
		// Chỉ load từ localStorage một lần nếu store đang rỗng
		if (!$backgroundUrl) {
		...
		// Lắng nghe thay đổi background từ tab khác
...
<Header />
<div
  class="flex min-h-screen flex-col items-center bg-fixed bg-center bg-no-repeat bg-cover relative"
  style="background-image: 
    {$backgroundUrl 
       ? `radial-gradient(circle, rgba(0,0,0,0.5) 0%, rgba(0,0,0,1) 100%), url('${$backgroundUrl}')` 
       : `radial-gradient(circle, rgba(11,13,25,0.5) 0%, rgba(11,13,25,0.9) 100%)`};"
>
	...
	<footer class="mt-4 flex flex-col md:flex-row w-full max-w-screen-xl items-center justify-between px-3 py-4 text-white/40 gap-4 md:gap-0">
		<!-- Trái -->
